...
if [[ ! ${IP_WAN_INTERFACE+x} ]]; then IP_WAN_INTERFACE="$(ip a | awk '/: / { print $2 }' | sed -n 3p | cut -d ':' -f1)" ; fi ; # 2nd interface 'eth0'
if [[ ! ${IP_WAN+x} ]]; then
...
IP_WAN2='' ;
if [[ ! ${IP_LB_INTERFACE+x} ]]; then IP_LB_INTERFACE="$(ip a | awk '/: / { print $2 }' | sed -n 4p | cut -d ':' -f1)" ; fi ; # // 2nd interface 'eth1'
...
if [[ ! ${IP_INTERFACE+x} ]]; then IP_INTERFACE="$(ip a | awk '/: / { print $2 }' | sed -n 2p | cut -d ':' -f1)" ; fi ; # // 2nd interface 'eth0'
if [[ ! ${IP_VAULT+x} && ${IP_INTERFACE} != "" ]]; then
	IP_VAULT1="$(ip a show ${IP_INTERFACE} | grep -oE '\b((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b' | head -n 1)" ;
fi ;
