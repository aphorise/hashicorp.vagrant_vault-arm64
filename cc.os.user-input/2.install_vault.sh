...
${0##*/} 0.0.9haproxy_dr-v1.14.1					17 August 2023
""" ;
fi ;
...
if [[ ! ${IP_LB_INTERFACE+x} ]]; then IP_LB_INTERFACE="$(ip a | awk '/: / { print $2 }' | sed -n 3p | cut -d ':' -f1)" ; fi ; # // 2nd interface 'eth2'
if [[ ! ${IP_WAN_INTERFACE+x} ]]; then IP_WAN_INTERFACE="$(ip a | awk '/: / { print $2 }' | sed -n 2p | cut -d ':' -f1)" ; fi ; # // 2nd interface 'eth1'
if [[ ! ${IP_LAN_INTERFACE+x} ]]; then IP_LAN_INTERFACE="$(ip a | awk '/: / { print $2 }' | sed -n 2p | cut -d ':' -f1)" ; fi ; # // 2nd interface 'eth1'
...
#	export VAULT_ADDR=${VAULT_API_ADDR} ;
#	$(grep 'VAULT_ADDR=' ${HOME_PATH}/.bashrc) ;
	export $(grep VAULT_ADDR .bashrc | cut -d' ' -f2)
